...
Now Playing uses a broad, off-screen ambient glow over the shared black base, without the focused
core glow. Its centre follows the artwork bottom and its radius exceeds the screen height so no
local light source is visible. An artwork-only alpha mask reveals the same background underneath,
avoiding an opaque black strip at the join. Album and artist detail heroes use that same image-only
fade, with no black continuation behind their metadata; their existing background glow and theme
ownership remain unchanged.
The lower controls distribute spare vertical room before progress, before transport, and before
